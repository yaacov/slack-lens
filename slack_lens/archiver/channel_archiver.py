...
from slack_lens.archiver.extractor import extract_visible_messages, find_scroller
...
class ChannelArchiver:
                        ...
                        if dom_timestamps:
                            try:
                                ...
                                if newest_dt < options.since:
                                    oldest = format_timestamp(
                                        str(min(dom_timestamps))
                                    )
                                    newest = format_timestamp(
                                        str(max(dom_timestamps))
                                    )
                                    console.print(
                                        f"  [yellow]All visible"
                                        f" msgs ({oldest}"
                                        f" - {newest}) are"
                                        f" before --since,"
                                        f" stopping"
    ...
    def _flush(
        ...
        *,
        ...
        """Build a ChannelArchive and delegate writing to Storage."""
        date_range = {}
        if messages:
        ...
        self.storage.save_channel(archive, filepath=filepath)
